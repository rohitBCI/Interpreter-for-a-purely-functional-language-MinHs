...
showSummary marks = color brightWhite $
    if length marks > 0
...
runTests exe testdir = do
    ...
    marks <- foreach tests $ (\test -> do
      ...
      putStr $ color brightWhite ("Running test: ") ++ color darkWhite (makeRelative testdir test)
                                                    ++ color brightWhite (" (worth "
                                                    ++ show mark ++ ") :-  ")
        ...
        then do
          ...
          if all (== B) $ map fst diff
            then putStrLn (color brightGreen $ name ++ " Check Passed!") >> return 1
              ...
              putStrLn $ (color brightRed $ name ++ " Check Failed") ++ ":\n" ++ showDiff diff;
              return 0
